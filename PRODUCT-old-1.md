...
26. **B26 — Wiki surface truthfulness.** The Wiki surface maintains local,
    private knowledge articles over the Fractorches recall corpus. Articles
    are user data in a Git-ignored local store and are never committed; corpus
    entries come only from the Fractorches recall API with review state and
    source-session provenance intact, and citations open the real session in
    Observatory. Every count, list, and status on the surface is measured from
    the article store or the corpus. A user-selected cluster of corpus entries
    can be compiled into a draft article by the Fractorches service's
    generation machinery; a draft is saved to the local store only after the
    user reviews it, and the article states which grounding entries changed
    since compilation, or says freshness is unknown when it cannot be checked.
    Article editing and any publication beyond the local store are absent
    until implemented.
