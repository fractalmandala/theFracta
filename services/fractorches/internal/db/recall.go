...
func NormalizeRecallQuery(q RecallQuery) RecallQuery {
	...
	q.DateFrom = strings.TrimSpace(q.DateFrom)
	q.DateTo = strings.TrimSpace(q.DateTo)
	q.Timezone = strings.TrimSpace(q.Timezone)
	q.ActiveSince = strings.TrimSpace(q.ActiveSince)
...
func ValidateRecallQuery(q RecallQuery) error {
	q = NormalizeRecallQuery(q)
	for _, date := range []string{q.Date, q.DateFrom, q.DateTo} {
		if date != "" && !timeutil.IsValidDate(date) {
			return fmt.Errorf(
				"%w: invalid date %q: use YYYY-MM-DD",
				ErrInvalidRecallQuery, date,
			)
		}
	}
	if q.DateFrom != "" && q.DateTo != "" && q.DateFrom > q.DateTo {
		return fmt.Errorf(
			"%w: date_from must not be after date_to",
			ErrInvalidRecallQuery,
		)
	}
	if q.ActiveSince != "" && !timeutil.IsValidTimestamp(q.ActiveSince) {
		return fmt.Errorf(
			"%w: invalid active_since %q: use RFC3339",
			ErrInvalidRecallQuery, q.ActiveSince,
		)
	}
	if _, err := NormalizeSessionTimezone(q.Timezone); err != nil {
		return fmt.Errorf("%w: %v", ErrInvalidRecallQuery, err)
	}
	switch q.Mode {
...
func buildRecallEntryWhere(q RecallQuery, includeText bool) (string, []any) {
	...
	if q.Model != "" || q.Date != "" || q.DateFrom != "" ||
		q.DateTo != "" || q.ActiveSince != "" || q.IncludeOneShot != nil ||
		q.IncludeAutomated != nil {
		...
		if q.IncludeOneShot != nil {
			sessionFilter.ExcludeOneShot = !*q.IncludeOneShot
		}
		if q.IncludeAutomated != nil {
			sessionFilter.ExcludeAutomated = !*q.IncludeAutomated
		}
		sessionWhere, sessionArgs := BuildSessionFilterPredicatesSQL(
			sessionFilter, SQLiteQueryDialect(), "recall_sessions",
		)
		preds = append(preds,
			"EXISTS (SELECT 1 FROM sessions recall_sessions WHERE "+
				"recall_sessions.id = recall_entries.source_session_id AND "+
				sessionWhere+")",
		)
