...
func (f SearchFilter) CanonicalSessionFilter() SessionFilter {
	visibility := SessionFilter{
		Project: f.Project, Agent: f.Agent, Model: f.Model,
		Date: f.Date, DateFrom: f.DateFrom, DateTo: f.DateTo,
		Timezone: f.Timezone, ActiveSince: f.ActiveSince,
		IncludeChildren: true,
	}
	if f.IncludeOneShot != nil {
		visibility.ExcludeOneShot = !*f.IncludeOneShot
	}
	if f.IncludeAutomated != nil {
		visibility.ExcludeAutomated = !*f.IncludeAutomated
	}
	return visibility
}
...
func (db *DB) Search(
	...
	visibility := f.CanonicalSessionFilter()
	scopeSQL, scopeArgs := BuildSessionFilterPredicatesSQL(
		visibility, SQLiteQueryDialect(), "s2")
	if scopeSQL != "" {
		innerWhere = append(innerWhere, scopeSQL)
		ftsArgs = append(ftsArgs, scopeArgs...)
	}
	nameScopeSQL, nameScopeArgs := BuildSessionFilterPredicatesSQL(
		visibility, SQLiteQueryDialect(), "s")
	if nameScopeSQL != "" {
		nameScopeSQL = "AND " + nameScopeSQL
	}
	...
	args := make([]any, 0, len(ftsArgs)*2+6+len(nameScopeArgs))
	...
	args = append(args, nameScopeArgs...)    // (7) optional name branch filters
	...
	query := fmt.Sprintf(`
		...
		nameScopeSQL,  // session filters for name branch (%s)
