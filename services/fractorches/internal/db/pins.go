...
type PinnedMessagesParams struct {
	SessionID                                     string
...
func (p PinnedMessagesParams) CanonicalSessionFilter() SessionFilter {
	f := SessionFilter{Project: p.Project, Agent: p.Agent, Model: p.Model,
		Date: p.Date, DateFrom: p.DateFrom, DateTo: p.DateTo,
		Timezone: p.Timezone, ActiveSince: p.ActiveSince, IncludeChildren: true}
	if p.IncludeOneShot != nil {
		f.ExcludeOneShot = !*p.IncludeOneShot
	}
	if p.IncludeAutomated != nil {
		f.ExcludeAutomated = !*p.IncludeAutomated
	}
	return f
}
...
func (db *DB) ListPinnedMessages(
	...
	return db.ListPinnedMessagesFiltered(ctx, PinnedMessagesParams{
		SessionID: sessionID, Project: project,
	})
}

func (db *DB) ListPinnedMessagesFiltered(
	ctx context.Context, p PinnedMessagesParams,
) ([]PinnedMessage, error) {
	var query string
	var args []any
	if p.SessionID != "" {
		...
		sessionWhere, sessionArgs := BuildSessionFilterPredicatesSQL(
			p.CanonicalSessionFilter(), SQLiteQueryDialect(), "s")
		if sessionWhere != "" {
			query += " WHERE " + sessionWhere
