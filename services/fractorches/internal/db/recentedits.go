...
func (p RecentEditsParams) CanonicalSessionFilter() SessionFilter {
...
ORDER BY fp.last_edited_at DESC NULLS LAST, fp.last_session_id DESC,
         ...
         r.rn`
	// Placeholders bind in text order: session filters, search, LIMIT, OFFSET,
	// then the per-file edit cap.
	qArgs := append([]any(nil), sessionArgs...)
	if p.Search != "" {
