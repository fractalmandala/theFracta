...
// BuildSessionFilterPredicatesSQL returns qualified user-facing predicates for
// queries that join another row set to canonical sessions.
func BuildSessionFilterPredicatesSQL(
	f SessionFilter, dialect QueryDialect, qualifier string,
) (string, []any) {
	return BuildSessionFilterPredicatesSQLAt(f, dialect, qualifier, 0)
}

// BuildSessionFilterPredicatesSQLAt is BuildSessionFilterPredicatesSQL for a
// query that already owns startIndex bound parameters. PostgreSQL callers use
// it to keep generated dollar placeholders contiguous with their local args.
func BuildSessionFilterPredicatesSQLAt(
	f SessionFilter, dialect QueryDialect, qualifier string, startIndex int,
) (string, []any) {
	b := NewQueryBuilder(dialect, startIndex)
...
func sessionFilterPredicates(
	...
	if f.Model != "" {
		models := splitCSV(f.Model)
		if len(models) == 1 {
			preds = append(preds, "EXISTS (SELECT 1 FROM messages model_messages WHERE model_messages.session_id = "+q("id")+" AND model_messages.model = "+b.Add(models[0])+")")
		} else if len(models) > 1 {
			placeholders := make([]string, len(models))
			for i, model := range models {
				placeholders[i] = b.Add(model)
			}
			preds = append(preds, "EXISTS (SELECT 1 FROM messages model_messages WHERE model_messages.session_id = "+q("id")+" AND model_messages.model IN ("+strings.Join(placeholders, ",")+"))")
		}
	}
	if f.Date != "" {
