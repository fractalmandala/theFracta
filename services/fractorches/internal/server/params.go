...
// parseOptionalBoolParam preserves the distinction between an omitted query
// parameter and an explicit false value. That distinction is required on
// list surfaces whose established default includes every session class.
func parseOptionalBoolParam(
	w http.ResponseWriter, r *http.Request, name string,
) (*bool, bool) {
	if !r.URL.Query().Has(name) {
		return nil, true
	}
	value, ok := parseBoolParam(w, r, name)
	if !ok {
		return nil, false
	}
	return &value, true
