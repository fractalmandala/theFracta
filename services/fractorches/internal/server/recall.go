...
func (s *Server) handleListRecallEntries(
	...
	includeOneShot, ok := parseOptionalBoolParam(w, r, "include_one_shot")
	if !ok {
		return
	}
	includeAutomated, ok := parseOptionalBoolParam(w, r, "include_automated")
	if !ok {
		return
	}
	if err := validateDateFilterValues(
		q.Get("date"), q.Get("date_from"), q.Get("date_to"),
		q.Get("active_since"),
	); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	timezone, err := db.NormalizeSessionTimezone(q.Get("timezone"))
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
