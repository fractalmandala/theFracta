...
func (s *Store) Search(ctx context.Context, f db.SearchFilter) (db.SearchPage, error) {
	...
	messageScope, messageScopeArgs := db.BuildSessionFilterPredicatesSQL(
		f.CanonicalSessionFilter(), db.DuckDBQueryDialect(), "s",
	)
	if messageScope != "" {
		messageScope = "AND " + messageScope
	}
	args = append(args, messageScopeArgs...)
	args = append(args, namePattern, namePattern, namePattern, namePattern)
	nameScope := messageScope
	if nameScope != "" {
		args = append(args, messageScopeArgs...)
	}
