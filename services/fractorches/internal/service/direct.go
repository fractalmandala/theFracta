...
func (b *directBackend) Search(
	...
	for _, value := range []struct {
		...
		{name: "date", date: req.Date},
		{name: "date_from", date: req.DateFrom},
		{name: "date_to", date: req.DateTo},
	} {
		if value.date != "" && !timeutil.IsValidDate(value.date) {
			return nil, &db.SearchInputError{Msg: fmt.Sprintf(
				"search: invalid %s %q: use YYYY-MM-DD",
				value.name, value.date,
			)}
		}
	}
	if req.DateFrom != "" && req.DateTo != "" && req.DateFrom > req.DateTo {
		return nil, &db.SearchInputError{
			Msg: "search: date_from must not be after date_to",
		}
	}
	if req.ActiveSince != "" && !timeutil.IsValidTimestamp(req.ActiveSince) {
		return nil, &db.SearchInputError{Msg: fmt.Sprintf(
			"search: invalid active_since %q: use RFC3339",
			req.ActiveSince,
		)}
	}
	...
	page, err := b.db.Search(ctx, db.SearchFilter{
		Query: db.PrepareFTSQuery(query), Project: req.Project,
		Agent: req.Agent, Model: req.Model, Date: req.Date,
		DateFrom: req.DateFrom, DateTo: req.DateTo, Timezone: req.Timezone,
		ActiveSince: req.ActiveSince, IncludeOneShot: req.IncludeOneShot,
		IncludeAutomated: req.IncludeAutomated, Sort: req.Sort,
		Cursor: req.Cursor, Limit: limit,
	})
	if err != nil {
		return nil, err
	}
