...
func TestDirectBackend_Search_ValidatesCanonicalSessionFilters(t *testing.T) {
	t.Parallel()
	d := dbtest.OpenTestDB(t)
	be := service.NewDirectBackend(d, nil)
	...
	tests := []struct {
		...
		{
			name: "invalid date",
			req:  service.SearchRequest{Query: "needle", Date: "2026-02-30"},
		},
		{
			name: "reversed range",
			req: service.SearchRequest{
				Query: "needle", DateFrom: "2026-08-20", DateTo: "2026-08-01",
			},
		},
		{
			name: "invalid active since",
			req:  service.SearchRequest{Query: "needle", ActiveSince: "yesterday"},
		},
		{
			name: "invalid timezone",
			req:  service.SearchRequest{Query: "needle", Timezone: "Mars/Olympus"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := be.Search(context.Background(), tt.req)
			require.Error(t, err)
			var inputErr *db.SearchInputError
			assert.True(t, errors.As(err, &inputErr),
				"invalid canonical filter should be a SearchInputError, got %T", err)
		})
	}
}
