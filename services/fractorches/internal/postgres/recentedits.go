...
ORDER BY fp.last_edited_at DESC NULLS LAST, fp.last_session_id DESC,
	...
	// Build $-placeholders in order: canonical session filters, search,
	// limit+1, offset, then the per-file edit cap.
	filterSQL, args := db.BuildSessionFilterPredicatesSQL(
	...
	q = fmt.Sprintf(q, filterSQL, search, limPH, offPH, kPH)
