...
func (s *Store) ListPinnedMessages(
	...
	return s.ListPinnedMessagesFiltered(ctx, db.PinnedMessagesParams{
		SessionID: sessionID, Project: project,
	})
}

func (s *Store) ListPinnedMessagesFiltered(
	ctx context.Context, p db.PinnedMessagesParams,
) ([]db.PinnedMessage, error) {
	var (
		rows *sql.Rows
		err  error
	...
	if p.SessionID != "" {
		...
		args := []any{}
		sessionWhere, sessionArgs := db.BuildSessionFilterPredicatesSQL(
			p.CanonicalSessionFilter(), db.PostgresQueryDialect(), "s")
		if sessionWhere != "" {
