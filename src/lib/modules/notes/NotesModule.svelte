<script lang="ts">
		...
		tick().then(() => {
			requestAnimationFrame(() => {
				...
				for (const el of elements) {
					const elLine = parseInt(
						el.getAttribute("data-source-line") || "0",
						10,
					);
					if (elLine >= savedLine) {
						target = el;
				...
				window.addEventListener("scrollend", clearRestoring, {
					once: true,
				});
	...
	let activePaginate = $derived(
		$docStore.frontmatter?.paginate === true ||
			$docStore.frontmatter?.paginate === "true",
	);
	...
	async function handleLocalLink(href: string) {
		...
		if (isExecutablePath(resolved)) {
			showToast(
				`Won't open executable file “${name}”. Open it from your file manager if you trust it.`,
			);
			return;
		}
	...
	async function handleCloseTab(id: string): Promise<boolean> {
		...
		if (t.dirty) {
			...
			const keepEditing = await ask(
				`You have unsaved changes to ${t.fileName}.`,
				{
	...
	onMount(() => {
		...
		(window as any).__fracta_open_file = () => {
			openVisible = true;
		};
		(window as any).__fracta_open_path = (path: string) => {
			if (path && rendererReady) {
				openFile(path);
			}
		};
		(window as any).__fracta_paste = () => {
			pasteDefaultMode = "paste";
			pasteVisible = true;
		};
		(window as any).__fracta_find = () => {
			searchVisible = !searchVisible;
		};
		(window as any).__fracta_zen = () => {
			zenMode = !zenMode;
		};
		...
		// a future menu open.
		(window as any).__fracta_ai_lookup = async (menuId: string) => {
	...
	function updateScrollDirection() {
		scrollDir =
			activeScrollKey === "j" ? 1 : activeScrollKey === "k" ? -1 : 0;
	}
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		if (
			e.ctrlKey &&
			(e.key === "Tab" || e.code === "PageUp" || e.code === "PageDown")
		) {
			e.preventDefault();
			const forward =
				e.key === "Tab" ? !e.shiftKey : e.code === "PageDown";
		...
		if ((e.metaKey || e.ctrlKey) && (e.key === "=" || e.key === "+")) {
			...
			settings.update((s) => ({
				...s,
				fontSize: Math.min(s.fontSize + 1, 32),
			}));
			return;
		}
	...
	$effect(() => {
		...
		if (prevTabId && prevTabId !== HOME_TAB_ID) {
			...
			if (
				prevTab &&
				!prevTab.isEditing &&
				!prevTab.filePath.startsWith("paste://")
			) {
				clearTimeout(scrollSaveTimer);
				if (
		...
		presenting =
			isMarpDoc(tab.frontmatter) &&
			get(settings).autoPresentMarp &&
			!tab.isEditing;

		setNativeWindowTitle(`${tab.fileName} — Fracta Knowledge`);
