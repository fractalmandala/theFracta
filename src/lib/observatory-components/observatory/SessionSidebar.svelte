...
<div class="panel min0 scroll-y box gap-3xs pad-y-2xs">
	<!-- Search row -->
	...
	{#if observatory.searchQuery.trim() && observatory.searchHits === null && !observatory.searching}
		<p class="text-xs text-muted pad-x-2xs">Press Search to query Fractorches. This list does not run a title-only substitute.</p>
	{/if}

	<!-- Sessions list -->
	<div class="min0 scroll-y navtree pad-x-2xs pad-y-3xs">
		{#each observatory.filteredSessions as session (session.id)}
			<button
				class="session-item navtree-link ta-l cursor-pointer"
				class:active={session.id === observatory.selectedSessionId}
			...
			>
				<span class="row ycenter xbetween gap-2xs">
					...
					<span class="text-sm text-primary text-2xs tt-u text-muted shrink-0">{session.agent}</span>
				</span>
				<span class="row ycenter gap-2xs text-2xs text-muted">
					<span class="mono text-xs text-muted truncate">{session.project || 'unknown'}</span>
				...
				{#if observatory.searchSnippets.get(session.id)}
					<small class="text-xs text-muted italic">{observatory.searchSnippets.get(session.id)}</small>
				{/if}
			</button>
		{:else}
			<p class="text-sm text-muted pad-x-2xs">No sessions match the current Fractorches query and filters.</p>
		{/each}
	</div>
</div>
